...
export const horizonObjectif = v.union(
  v.literal("court"),
  v.literal("moyen"),
  v.literal("long"),
);
export const priorite = v.union(
  v.literal("basse"),
  v.literal("normale"),
  v.literal("haute"),
);
export const statutObjectif = v.union(
  v.literal("actif"),
  v.literal("pause"),
  v.literal("archive"),
);
...
export default defineSchema({
  ...
  onboarding: defineTable({
    ...
    tonCopilote: v.optional(
      v.union(v.literal("doux"), v.literal("direct"), v.literal("equilibre")),
    ),
  ...
  etapesObjectif: defineTable({
    profilId: v.id("profils"),
    objectifId: v.id("objectifs"),
    titre: v.string(),
    ordre: v.number(),
    termineeLe: v.optional(v.number()),
  })
  ...
  revuesQuotidiennes: defineTable({
    profilId: v.id("profils"),
    journeeId: v.id("journees"),
    energie: v.optional(v.number()),
    blocage: v.optional(v.string()),
    note: v.optional(v.string()),
    termineeLe: v.optional(v.number()),
    misAJourLe: v.number(),
  })
  ...
  messagesConversation: defineTable({
    ...
    role: v.union(v.literal("utilisateur"), v.literal("assistant")),
    contenu: v.string(),
    statut: v.union(
      v.literal("envoye"),
  ...
  propositionsMemoire: defineTable({
    ...
    categorie: v.union(
      ...
      v.literal("observationTemporaire"),
    ),
    contenu: v.string(),
    source: v.string(),
    statut: v.union(
  ...
  occurrencesHabitude: defineTable({
    ...
    .index("by_profil_and_date", ["profilId", "date"])
    .index("by_habitude_and_date", ["habitudeId", "date"]),

  operationsAppliquees: defineTable({
    profilId: v.id("profils"),
    operationId: v.string(),
    type: v.string(),
    resultatJson: v.string(),
    appliqueLe: v.number(),
  }).index("by_profil_and_operation", ["profilId", "operationId"]),
  ...
  historiqueModifications: defineTable({
    ...
    entiteId: v.string(),
    operationId: v.string(),
    avantJson: v.optional(v.string()),
    apresJson: v.optional(v.string()),
    motif: v.string(),
    creeLe: v.number(),
  }).index("by_profil_and_date", ["profilId", "creeLe"]),

  retoursInsightsProgression: defineTable({
    profilId: v.id("profils"),
    insightId: v.string(),
    semaine: v.string(),
    retour: v.union(v.literal("faux"), v.literal("inutile")),
    misAJourLe: v.number(),
  }).index("by_profil_and_insight", ["profilId", "insightId"]),

  preferencesNotifications: defineTable({
    profilId: v.id("profils"),
    briefingMatin: v.boolean(),
    heureBriefing: v.string(),
    revueSoir: v.boolean(),
    heureRevue: v.string(),
    rappelPriorite: v.boolean(),
    pauseJusqua: v.optional(v.number()),
    fuseauHoraire: v.string(),
    misAJourLe: v.number(),
  }).index("by_profil", ["profilId"]),

  droitsAbonnement: defineTable({
    authId: v.string(),
    ...
    misAJourLe: v.number(),
  }).index("by_auth_and_entitlement", ["authId", "entitlement"]),

  evenementsRevenueCat: defineTable({
