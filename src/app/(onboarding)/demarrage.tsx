...
export default function Onboarding() {
  ...
  return (
    <OnboardingShell step={step}>
      ...
      {step === 2 && <View>{draft.objectifs.map((goal, i) => <Field key={i} label={t("onboarding.objectifLabel", { number: i + 1 })} onChangeText={value => setDraft(d => ({ ...d, objectifs: d.objectifs.map((x, index) => index === i ? value : x) }))} placeholder={t("onboarding.objectifPlaceholder")} value={goal} />)}{draft.objectifs.length < 3 && <Button variant="ghost" label={t("onboarding.ajouterObjectif")} onPress={() => setDraft(d => ({ ...d, objectifs: [...d.objectifs, ""], durees: [...d.durees, 30] }))} />}</View>}
      ...
      {step === 6 && <View>{cleanGoals.map((goal, i) => <View key={i} className="mb-4"><Card tone={i === 0 ? "impulse" : "default"}><TextInput accessibilityLabel={t("onboarding.plan.action", { number: i + 1 })} className="font-medium text-base text-ink" onChangeText={value => setDraft(d => ({ ...d, objectifs: d.objectifs.map((x, index) => index === i ? value : x) }))} value={goal} /><View accessibilityRole="radiogroup" className="mt-3 flex-row flex-wrap items-center"><Text className="mr-3 text-sm text-muted">{t("onboarding.plan.duree")}</Text>{[15,30,45,60].map(n => <Pressable key={n} accessibilityLabel={`${t("onboarding.plan.duree")} ${n}`} accessibilityRole="radio" accessibilityState={{ checked: (draft.durees[i] ?? 30) === n }} className="mr-2 min-h-touch min-w-touch items-center justify-center rounded border border-line px-2" onPress={() => setDraft(d => ({ ...d, durees: d.durees.map((x,index) => index === i ? n : x) }))}><Text className={(draft.durees[i] ?? 30) === n ? "text-action" : "text-muted"}>{n}</Text></Pressable>)}</View></Card></View>)}</View>}
      <View className="mt-auto flex-row pt-8">
        {step > 0 && <View className="mr-3 flex-1"><Button variant="secondary" disabled={busy} label={t("onboarding.retour")} onPress={() => persist(step - 1)} /></View>}
        <View className="flex-1"><Button disabled={!valid()} loading={busy} loadingLabel={t("onboarding.sauvegarde")} label={step === 6 ? t("onboarding.validerPlan") : t("onboarding.continuer")} onPress={() => step === 6 ? complete() : persist(step + 1)} /></View>
      </View>
