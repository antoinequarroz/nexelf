...
export default function Onboarding() {
  ...
  return (
    <OnboardingShell step={step}>
      ...
      {step === 6 && <View>{cleanGoals.map((goal, i) => <View key={i} className="mb-4"><Card tone={i === 0 ? "impulse" : "default"}><TextInput accessibilityLabel={t("onboarding.plan.action", { number: i + 1 })} className="font-medium text-base text-ink" onChangeText={value => setDraft(d => ({ ...d, objectifs: d.objectifs.map((x, index) => index === i ? value : x) }))} value={goal} /><View className="mt-3 flex-row flex-wrap items-center"><Text className="mr-3 text-sm text-muted">{t("onboarding.plan.duree")}</Text>{[15,30,45,60].map(n => <Pressable key={n} accessibilityRole="radio" accessibilityState={{ checked: (draft.durees[i] ?? 30) === n }} className="mr-2 min-h-11 justify-center rounded border border-line px-2" onPress={() => setDraft(d => ({ ...d, durees: d.durees.map((x,index) => index === i ? n : x) }))}><Text className={(draft.durees[i] ?? 30) === n ? "text-action" : "text-muted"}>{n}</Text></Pressable>)}</View></Card></View>)}</View>}
