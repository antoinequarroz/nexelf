...
export default function Onboarding() {
  ...
  return (
    <OnboardingShell step={step}>
      ...
      {step === 4 && <View className="flex-row justify-between">{[1,2,3,4,5].map(x => <ChoiceChip key={x} label={`${x}`} selected={draft.energie === x} onPress={() => setDraft(d => ({ ...d, energie: x }))} />)}</View>}
      {step === 5 && <View>{(["doux","equilibre","direct"] as Tone[]).map(x => <ChoiceChip key={x} label={t(`onboarding.tons.${x}`)} selected={draft.ton === x} onPress={() => setDraft(d => ({ ...d, ton: x }))} />)}</View>}
